...
SERVER_PORT = 8800  # TECH_SPEC: Gateway Port
...
def save_key(key: str, value: str) -> None:
    ...
    with open(ENV_FILE, "a", encoding="utf-8") as f:
        f.write(f"\n{key}={value}\n")
    print(f"  [AUTO] {key} generated and saved to .env.")
...
def preflight_check() -> bool:
    ...
    """
    print("=" * 60)
    print("  MONEWMENT-0 Pre-Flight Environment Check")
    print("=" * 60)
    ...
    for key, desc, auto_gen in REQUIRED_KEYS:
        if key in current_env and current_env[key]:
            ...
            print(f"  [FAIL] {key} ({desc}) — .env 에 반드시 직접 설정해야 합니다.")
...
def start_server(port: int = SERVER_PORT) -> None:
    """Uvicorn 으로 MONEWMENT-0 서버를 시작합니다."""
    print(f"\n  [*] Starting MONEWMENT-0 on port {port}...")
    os.chdir(Path(__file__).resolve().parent)  # main.py 가 있는 디렉터리로 이동
    subprocess.run([
