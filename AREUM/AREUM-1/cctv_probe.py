"""
cctv_probe.py — CCTV Probe (AREUM Template)
[G8] TECH_SPEC 위반 교정:
...
class ProbeSettings(BaseSettings):
    ...
    PORT_CORE_API: str = "8800" # AREUM은 EDENVALE 기본(8800)에 맞춰 세팅 
    model_config = SettingsConfigDict(env_file=".env", extra="ignore")


_ALLOWED_LEVELS = {"QUEEN", "ANT", "STRATUM", "MONEWMENT", "AREUM"}


def run_cctv_probe(level: str) -> None:
    """[Decentralized CCTV Probe] Fetches live docs and saves as LIVE_CONTEXT.md."""
    ...
    logger.info(f"Synchronizing {level_upper} context...")

    try:
        probe_settings = ProbeSettings()
    except Exception as e:
        logger.error(f"Settings load failed — is CCTV_SECRET set in .env? Error: {e}")
        sys.exit(1)
    ...
    try:
        ...
        logger.warning(f"Failed to reach Core ({e}). Using cached documentation.")


if __name__ == "__main__":
    if len(sys.argv) < 2:
        logger.error("Usage: python cctv_probe.py {queen|ant|stratum|monewment|areum}")
        sys.exit(1)
